...
const { mapKeys, isPlainObject, trimEnd, trimStart, compact } = require('lodash')
...
class WordPressSource {
  ...
  constructor (api, options) {
    ...
    api.loadSource(async (actions, store) => {
      ...
      const test_collection = actions.addCollection("test")
      const test_collection_2 = actions.addCollection("test2")
      const test = test_collection_2.addNode({id:3})
      

      test_collection.addNode({ id: 0, ref: actions.store.createReference(test) });
  ...
  normalizeFields (fields) {
    ...
    //check if fields is a component
    if(fields.acf_fc_layout) {
      let collection = this.store.getCollection(`flexible_${fields.acf_fc_layout}`)
      if(collection == undefined) {
        collection = this.store.addCollection(`flexible_${fields.acf_fc_layout}`)
      }
      const new_component = collection.addNode(fields)
      return this.store.store.createReference(new_component)
    }
